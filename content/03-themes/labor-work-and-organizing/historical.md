---
title: Historical
resources:
  - title: "Strike!"
    authors: ["Jeremy Brecher"]
    year: 1972
    format: reading
    kind: book
    access: free
    url: "https://libcom.org/article/strike-jeremy-brecher"
    note: "A narrative history of US mass strikes from 1877 to 1970 told from the rank and file's point of view. The backbone text for this lens."
  - title: "The Making of the English Working Class"
    authors: ["E. P. Thompson"]
    year: 1963
    format: reading
    kind: book
    access: borrow
    url: "https://archive.org/details/makingofenglishw0000epth"
    note: "The founding work of history from below, on how workers made themselves into a class through struggle, custom, and culture rather than being made by economic forces alone."
  - title: "History of the Labor Movement in the United States, Vol. 1"
    authors: ["Philip S. Foner"]
    year: 1947
    format: reading
    kind: book
    access: borrow
    url: "https://archive.org/details/historyoflabormo0001fone"
    note: "The opening volume of the standard left history of US labor: early unions, the ten-hour movement, the Knights of Labor, and the 1877 uprising."
  - title: "The Great Labor Uprising of 1877"
    authors: ["Philip S. Foner"]
    year: 1977
    format: reading
    kind: book
    access: borrow
    url: "https://archive.org/details/greatlaboruprisi0000fone"
    note: "Foner's focused account of the first national strike wave, useful for studying a single generative event closely. Note the school previously listed this under a title that does not exist, pointing at an excerpt of his Volume 1."
  - title: "Labor's Untold Story"
    authors: ["Richard O. Boyer", "Herbert M. Morais"]
    year: 1955
    format: reading
    kind: book
    access: borrow
    url: "https://archive.org/details/laborsuntoldstor0000boye"
    note: "A popular union-published history of American workers from the Civil War through the New Deal, written to be read by workers themselves."
  - title: "Workers' Control in America"
    authors: ["David Montgomery"]
    year: 1979
    format: reading
    kind: book
    access: paywalled
    url: "https://www.cambridge.org/us/universitypress/subjects/history/american-history-general-interest/workers-control-america-studies-history-work-technology-and-labor-struggles?format=PB&isbn=9780521280068"
    note: "How nineteenth and twentieth century workers fought for control over production, a bridge from labor history into the labor process."
  - title: "David Montgomery: Labor Struggles, Past, Present, Future"
    authors: ["David Montgomery"]
    format: video
    kind: lecture
    access: free
    length: "114 min"
    url: "https://www.youtube.com/watch?v=l6CHjRTAblI"
    note: "About 4,300 views, a very niche pick. Montgomery on the long arc of workers' self-activity and shop-floor struggle, which is the book's subject. He died in 2011 and the recorded record is thin, so this is what exists."
  - title: "William D. Haywood: selected writings, including The General Strike"
    authors: ["William D. Haywood"]
    year: 1911
    format: primary-source
    kind: collection
    access: free
    url: "https://www.marxists.org/archive/haywood-b/index.htm"
    note: "Primary sources from a founder of the IWW, giving a first-person voice to revolutionary industrial unionism."
  - title: "National Labor Relations Act (Wagner Act)"
    authors: ["U.S. Congress"]
    year: 1935
    format: primary-source
    kind: document
    access: free
    url: "https://www.archives.gov/milestone-documents/national-labor-relations-act"
    note: "The statute that legalized collective bargaining and created the NLRB. The essential document for how labor law shaped and constrained the union form."
  - title: "The Industrial Economy: Crash Course US History #23"
    authors: ["CrashCourse", "John Green"]
    year: 2013
    format: video
    kind: explainer
    access: free
    length: "13 min"
    url: "https://www.youtube.com/watch?v=r6tRp-zRUJs"
    note: "About 4 million views. A fast survey of post-Civil-War industrialization, the Knights of Labor, the AFL, and early strikes. Mainstream framing to complicate."
  - title: "The Wobblies: Iconic Film on the IWW Rereleased for May Day"
    authors: ["Democracy Now!"]
    year: 2022
    format: video
    kind: interview
    access: free
    length: "17 min"
    url: "https://www.youtube.com/watch?v=t0ruK6IQLWc"
    note: "About 54,000 views. Clips and interviews on the Industrial Workers of the World and its repression. A short historical entry to radical unionism."
  - title: "Black Reconstruction in America"
    authors: ["W.E.B. Du Bois"]
    year: 1935
    format: reading
    kind: book
    access: borrow
    url: "https://archive.org/details/blackreconstruct0000dubo"
    note: "Chapter 4, The General Strike, is the labor history claim: enslaved people withdrew their labor from the Confederacy and broke it, which makes them workers who struck rather than people who were freed. Du Bois writing as a Black Marxist against the white supremacist Dunning School consensus of his day."
  - title: "The Many-Headed Hydra"
    authors: ["Peter Linebaugh", "Marcus Rediker"]
    year: 2000
    format: reading
    kind: book
    access: free
    url: "https://files.libcom.org/files/Linebaugh%20and%20Rediker%20-%20The%20Many-Headed%20Hydra%20-%20Sailors,%20Slaves,%20Commoners,%20and%20the%20Hidden%20H.pdf"
    note: "The sailors, slaves, and commoners of the revolutionary Atlantic as a single multi-ethnic working class, organizing across every line the authorities used to divide them. History from below, and a direct answer to any account of labor that begins with the white male craft worker."
  - title: "The 1973 Durban Strikes: Building Popular Democratic Power in South Africa"
    authors: ["Tricontinental: Institute for Social Research"]
    year: 2023
    format: reading
    kind: article
    access: free
    url: "https://thetricontinental.org/dossier-1973-durban-strikes/"
    note: "A 44-page dossier on the wildcat strikes that revived the South African labor movement under apartheid and led to COSATU. The clearest short account of workers organizing where the union itself was effectively illegal. Tricontinental is an openly Marxist research institute; label the standpoint and use the narrative."
  - title: "Labour Rebellions of the 1930s in the British Caribbean Region Colonies"
    authors: ["Richard Hart"]
    year: 2002
    format: reading
    kind: article
    access: free
    url: "https://libcom.org/article/labour-rebellions-1930s-british-caribbean-region-colonies-richard-hart"
    note: "The 1935 to 1938 strike wave across the British Caribbean, which produced both the region's unions and its independence movements at the same time. Hart was a Jamaican communist and a participant in the movement he describes, so this is a partisan insider's history."
  - title: "A New System of Slavery: The Export of Indian Labour Overseas, 1830-1920"
    authors: ["Hugh Tinker"]
    year: 1974
    format: reading
    kind: book
    access: borrow
    url: "https://archive.org/details/newsystemofslave0000tink"
    note: "How Britain moved a million and a half indentured Indian workers around the empire after abolition, and whether the contract they signed made them free. Tinker is a liberal anti-colonial historian, and his title thesis, that indenture was slavery renamed, is contested by later scholars who stress migrant agency."
  - title: "Charisma's Birth from the Bottom Up: Lula, ABC's Metalworkers' Strikes and the Social History of Brazilian Politics"
    authors: ["John D. French"]
    year: 2022
    format: reading
    kind: article
    access: free
    url: "https://www.cambridge.org/core/journals/journal-of-latin-american-studies/article/charismas-birth-from-the-bottom-up-lula-abcs-metalworkers-strikes-and-the-social-history-of-brazilian-politics/E7E1D10F63BD31C81F190FD917D56267"
    note: "A 25-page open-access account of the 1978 to 1980 strikes in the ABC belt of Sao Paulo, where metalworkers led by a lathe operator named Lula defied a military dictatorship and went on to found a workers' party. Shows shopfloor authority built from the bottom up rather than conferred by a charismatic leader. French is a labor historian sympathetic to Lula and the PT."
  - title: "Chinese Railroad Workers in North America Project"
    authors: ["Stanford University"]
    format: primary-source
    kind: archive
    access: free
    url: "https://web.stanford.edu/group/chineserailroad/cgi-bin/website/"
    note: "Recovers the roughly 20,000 Chinese workers who built the western half of the transcontinental railroad, including their 1867 strike, one of the largest of its century, which is absent from most labor histories. An academic recovery project working largely without the workers' own written records, which is itself the lesson."
  - title: "Delano Manongs: Forgotten Heroes of the United Farm Workers"
    authors: ["Marissa Aroy"]
    year: 2014
    format: film
    kind: documentary
    access: free
    length: "27 min"
    url: "https://www.pbs.org/video/kvie-viewfinder-delano-manongs/"
    note: "The Filipino farmworkers who started the 1965 Delano grape strike and asked Chavez's mostly Mexican union to join them, a fact the standard telling of the UFW drops. Made by a Filipina American filmmaker to correct that record. Free on PBS."
  - title: "Striking Back: A History of COSATU"
    authors: ["Jeremy Baskin"]
    year: 1991
    format: reading
    kind: book
    access: borrow
    tier: extended
    url: "https://archive.org/details/bwb_P7-ERJ-898"
    note: "The full history of the federation that became the industrial wing of the fight against apartheid, and then governed alongside the ANC. Baskin was COSATU's own information officer, so this is an insider account committed to the alliance, and best read knowing that."
  - title: "Solidarnosc: Trade Unionism or Self-Organisation?"
    authors: ["Internationalist Communist Tendency"]
    year: 2019
    format: reading
    kind: article
    access: free
    tier: extended
    url: "https://libcom.org/article/solidarnosc-trade-unionism-or-self-organisation"
    note: "Argues the 1980 Polish mass strikes were a genuine workers' self-organization that the Solidarity union then contained and disarmed. An openly left-communist polemic hostile to unions as a form; useful precisely because it puts that argument at its sharpest against the rest of this lens."
  - title: "Labor Studies research guide (Catherwood Library)"
    authors: ["Cornell University ILR School"]
    format: reading
    kind: collection
    access: free
    tier: extended
    url: "https://guides.library.cornell.edu/laborstudies"
    note: "The gateway to the largest labor archive in North America: finding aids, digitized collections, and where to look next. The place to start if a group wants to research its own local labor history."
---

_How did this come to be? What were the conditions, struggles, and contingencies that produced the current arrangement?_

See [[02-pedagogy#Historical|the pedagogy document]] for what this lens does across all themes.

The job, the union, the weekend, the strike, and the law that governs them were all made, and most of them recently. This lens denaturalizes the world of work by tracing how the working class was formed, how it organized, and how the ground rules were set through struggle, victory, and defeat. It builds directly on [[03-themes/how-capitalism-works/historical|the historical lens of the capitalism theme]], which covers how people came to have nothing to sell but their ability to work.

## Anchor questions

- Where did the eight-hour day, the weekend, and the minimum wage come from? Who fought for them, and who died for them?
- If most people once worked land or trades, how did they end up working for wages? What did that take?
- Unions did not always exist. When and how were they built, and against what?
- Why is so much labor law written the way it is? Whose strikes and defeats shaped it?
- Whose labor history gets told, and whose gets left out?
- Were enslaved people workers? If they withdrew their labor and broke the Confederacy by doing it, what was that, if not a strike?
- In the colonies, the fight against the boss and the fight against the empire were the same fight. Why did they come apart in the United States?

## Materials

Free unless marked otherwise. "Free to borrow" means a free Internet Archive account and a renewable one-hour loan.

### Books

- **Jeremy Brecher** (b. 1946), _Strike!_ (1972, updated later). A narrative history of US mass strikes from 1877 to 1970 told from the rank and file's point of view, covering the 1877 railroad uprising, the 1934 city-wide strikes, and the 1946 strike wave. The best single-volume entry point for this lens. Free: [libcom.org](https://libcom.org/article/strike-jeremy-brecher)
- **E. P. Thompson** (1924 to 1993), _The Making of the English Working Class_ (1963). The founding work of "history from below," reconstructing how English workers made themselves into a class through struggle, custom, and culture between 1780 and 1832, rather than being made by economic forces alone. Long and endlessly readable. Free to borrow: [archive.org](https://archive.org/details/makingofenglishw0000epth)
- **Philip S. Foner** (1910 to 1994), _History of the Labor Movement in the United States, Volume 1_ (1947). The opening volume of the standard left history of American labor, from colonial times to the founding of the AFL, covering early unions, the ten-hour movement, the Knights of Labor, and the 1877 uprising. Free to borrow: [archive.org](https://archive.org/details/historyoflabormo0001fone)
- **Philip S. Foner**, _The Great Labor Uprising of 1877_ (1977). A focused account of the first national strike wave, drawn from Foner's larger history and useful when a session wants to study one generative event closely. Free to borrow: [archive.org](https://archive.org/details/greatlaboruprisi0000fone)
- **Richard O. Boyer** (1903 to 1973) and **Herbert M. Morais** (1905 to 1970), _Labor's Untold Story_ (1955). A popular union-published history of American workers from the Civil War through the New Deal, written to be read by workers themselves. Partisan and readable. Free to borrow: [archive.org](https://archive.org/details/laborsuntoldstor0000boye)
- **David Montgomery** (1927 to 2011), _Workers' Control in America_ (1979). Essays on how workers fought for control over production and how management fought back, connecting shop-floor struggle to the rise of the modern firm. A bridge from labor history into the theoretical lens. No borrowable copy exists, and Cambridge blocks automated checks though the page is live. Paywalled: [cambridge.org](https://www.cambridge.org/us/universitypress/subjects/history/american-history-general-interest/workers-control-america-studies-history-work-technology-and-labor-struggles?format=PB&isbn=9780521280068). A free Montgomery lecture survives, very niche at about 4,300 views; he died in 2011 and the record is thin: [youtube.com](https://www.youtube.com/watch?v=l6CHjRTAblI)

### Unfree labor, and the workers the story leaves out

The books above tell the story through the free wage worker, who was mostly white and mostly male, and through the unions that admitted him. That worker was never the majority of anyone's workforce. This section puts the people the standard account leaves out back where they belong, which is at the center.

- **W.E.B. Du Bois** (1868 to 1963), _Black Reconstruction in America_ (1935). Read Chapter 4, "The General Strike," for this theme: Du Bois's argument that enslaved people withdrew their labor from the Confederacy and destroyed it, which makes them workers who struck rather than people to whom freedom was granted. The most important labor history claim in American historiography, and it is not in most labor histories. Du Bois wrote it as a Black Marxist against the white supremacist consensus of his profession. Shared with [[03-themes/anti-blackness-and-racial-capitalism/historical|the anti-Blackness theme]]. Free to borrow: [archive.org](https://archive.org/details/blackreconstruct0000dubo)
- **Peter Linebaugh** (b. 1942) and **Marcus Rediker** (b. 1951), _The Many-Headed Hydra_ (2000). The sailors, slaves, and commoners of the revolutionary Atlantic as one multi-ethnic working class, organizing across every line the authorities drew to divide them. Shared with [[03-themes/how-capitalism-works/historical|the capitalism theme]]. Free PDF: [files.libcom.org](https://files.libcom.org/files/Linebaugh%20and%20Rediker%20-%20The%20Many-Headed%20Hydra%20-%20Sailors,%20Slaves,%20Commoners,%20and%20the%20Hidden%20H.pdf)
- **Hugh Tinker** (1921 to 2000), _A New System of Slavery: The Export of Indian Labour Overseas, 1830-1920_ (1974). How Britain moved more than a million indentured Indian workers to Mauritius, Natal, Fiji, and the Caribbean to replace the enslaved after abolition, and whether the contract they signed made them free. The category the room needs is the unfree waged worker. Tinker writes as a liberal anti-colonial historian, and his title thesis, that indenture was slavery renamed, is contested by later scholars who argue it erases the migrants' own agency. Free to borrow: [archive.org](https://archive.org/details/newsystemofslave0000tink)
- **Chinese Railroad Workers in North America Project** (Stanford University). Recovers the roughly 20,000 Chinese workers who built the western half of the transcontinental railroad, including their strike of 1867, among the largest of that century and absent from most accounts of it. The project works largely without the workers' own written records, which is itself the lesson about whose labor history survives. Free: [web.stanford.edu](https://web.stanford.edu/group/chineserailroad/cgi-bin/website/)
- **Delano Manongs: Forgotten Heroes of the United Farm Workers** (Marissa Aroy, 2014, 27 min). Larry Itliong and the Filipino farmworkers who walked out of the Delano vineyards in September 1965 and then asked Chavez's mostly Mexican union to join them, which is how the UFW happened. The standard telling drops them. Made by a Filipina American documentarian recovering her own community's history, affectionate toward the manongs and pointed about their erasure. Free on PBS: [pbs.org](https://www.pbs.org/video/kvie-viewfinder-delano-manongs/)

### Labor outside the United States and Britain

Everything above happens in two countries. The pattern elsewhere is different in a way that matters: where the state was colonial or authoritarian, the labor question and the national question were the same question, and the strike was the thing that broke the regime.

- **The 1973 Durban Strikes: Building Popular Democratic Power in South Africa** (Tricontinental, 2023). A 44-page dossier on the wildcat strikes that pulled some 60,000 African workers out across more than 100 firms and revived a movement the apartheid state believed it had crushed, running from Durban to FOSATU to COSATU. The clearest short account of organizing where the union was effectively illegal. Tricontinental is an avowedly Marxist institute writing in a movement-building register, sympathetic to the South African Communist Party tradition. Free: [thetricontinental.org](https://thetricontinental.org/dossier-1973-durban-strikes/)
- **Richard Hart** (1906 to 2006), _Labour Rebellions of the 1930s in the British Caribbean Region Colonies_ (2002). The strike wave from the St Kitts sugar workers in 1935 to the Jamaican rebellion of 1938, where colonial police killed dozens, and how those rebellions produced the region's unions and its independence parties together. Hart was a Jamaican communist who helped build both, so he writes as a partisan of the strikers and insists on the word rebellion over riot. Free: [libcom.org](https://libcom.org/article/labour-rebellions-1930s-british-caribbean-region-colonies-richard-hart)
- **John D. French**, _Charisma's Birth from the Bottom Up_ (2022). Twenty-five open-access pages on the strikes of 1978 to 1980 in the ABC industrial belt of Sao Paulo, where metalworkers led by a lathe operator named Lula defied a military dictatorship, built a union independent of the state, and founded a party. A case of shopfloor authority made from below rather than handed down. French is sympathetic to Lula and the PT. Free: [cambridge.org](https://www.cambridge.org/core/journals/journal-of-latin-american-studies/article/charismas-birth-from-the-bottom-up-lula-abcs-metalworkers-strikes-and-the-social-history-of-brazilian-politics/E7E1D10F63BD31C81F190FD917D56267)

### Primary sources

- **William D. "Big Bill" Haywood** (1869 to 1928), selected writings including _The General Strike_ (1911). First-person sources from a founder of the IWW and the Western Federation of Miners, giving voice to revolutionary industrial unionism at its height. Free: [marxists.org](https://www.marxists.org/archive/haywood-b/index.htm)
- **U.S. Congress**, _National Labor Relations Act (Wagner Act)_ (1935). The full statutory text that legalized collective bargaining and created the National Labor Relations Board. The essential document for understanding how labor law both protected and constrained the union form, and worth reading against Taft-Hartley (1947). Free: [archives.gov](https://www.archives.gov/milestone-documents/national-labor-relations-act)

### Films and documentaries

- **"The Industrial Economy: Crash Course US History #23"** (CrashCourse, John Green, 2013). Roughly 13 minutes, about 4 million views, the most-watched item across this theme. A fast survey of post-Civil-War industrialization, the Knights of Labor, the AFL, and early strikes. Mainstream framing, useful as a shared starting reference to then complicate. [youtube.com/watch?v=r6tRp-zRUJs](https://www.youtube.com/watch?v=r6tRp-zRUJs)
- **"The Wobblies: Iconic Film on the IWW Rereleased for May Day"** (Democracy Now!, 2022). Roughly 17 minutes, about 54,000 views. A segment of clips and interviews on the Industrial Workers of the World and the repression it faced. A short entry to radical industrial unionism, best paired with the Haywood readings. [youtube.com/watch?v=t0ruK6IQLWc](https://www.youtube.com/watch?v=t0ruK6IQLWc)

<!-- further-reading:start -->

## Further reading

_For resources behind a paywall, see [[04-resources/index#A note on access|the note on access]]._

- **Jeremy Baskin**, _Striking Back: A History of COSATU_ (1991). The full history of the federation that became the industrial wing of the fight against apartheid, and then governed alongside the ANC. Baskin was COSATU's own information officer, so this is an insider account committed to the alliance, and best read knowing that. Free to borrow: [archive.org](https://archive.org/details/bwb_P7-ERJ-898)
- **Internationalist Communist Tendency**, _Solidarnosc: Trade Unionism or Self-Organisation?_ (2019). Argues the 1980 Polish mass strikes were a genuine workers' self-organization that the Solidarity union then contained and disarmed. An openly left-communist polemic hostile to unions as a form; useful precisely because it puts that argument at its sharpest against the rest of this lens. Free: [libcom.org](https://libcom.org/article/solidarnosc-trade-unionism-or-self-organisation)
- **Cornell University ILR School**, _Labor Studies research guide (Catherwood Library)_. The gateway to the largest labor archive in North America: finding aids, digitized collections, and where to look next. The place to start if a group wants to research its own local labor history. Free: [guides.library.cornell.edu](https://guides.library.cornell.edu/laborstudies)

<!-- further-reading:end -->

## Status

First pass, and it may be a while before there is a second. Contributions welcome, especially on Indian and South Asian labor history, which remains the thinnest area here, and on labor in the socialist states and in the Middle East, which this lens does not yet reach at all. Note also that China Labour Bulletin, long the standard source on Chinese labor, was forced to close in 2025 and its site is gone; a replacement source would be valuable.
