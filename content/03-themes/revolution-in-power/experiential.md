---
title: Experiential
resources:
  - title: "Secondhand Time: The Last of the Soviets"
    authors: ["Svetlana Alexievich"]
    year: 2013
    format: reading
    kind: book
    access: paywalled
    url: "https://www.penguinrandomhouse.com/books/541184/secondhand-time-by-svetlana-alexievich/"
    note: "The Nobel laureate's assembled testimony of ordinary people living through the Soviet collapse, holding pride, loss, terror, and longing together. The definitive balanced experiential source."
  - title: "On the Battle Lost (Nobel Lecture)"
    authors: ["Svetlana Alexievich"]
    year: 2015
    format: reading
    kind: lecture
    access: free
    url: "https://www.nobelprize.org/prizes/literature/2015/alexievich/lecture/"
    note: "Her Nobel lecture, delivered as Secondhand Time was being finished, on the Red Man and the wreckage of the Soviet century. Free, permanent, and assignable as a reading. The best companion in the school."
  - title: "Hope Against Hope: A Memoir"
    authors: ["Nadezhda Mandelstam"]
    year: 1970
    format: reading
    kind: book
    access: borrow
    url: "https://archive.org/details/hopeagainsthopem00mand"
    note: "The widow of the poet Osip Mandelstam recounts his arrest and death in the Great Purge. A canonical first-person account of Stalin-era repression, essential for the critical side of the balance."
  - title: "Reimagining Soviet Georgia"
    authors: ["Sopiko Japaridze", "Bryan Gigantino"]
    year: 2020
    format: podcast
    kind: series
    access: free
    url: "https://podcasts.apple.com/us/podcast/reimagining-soviet-georgia/id1555761205"
    note: "A Tbilisi-based project gathering oral histories on daily life, socialism, and post-socialism in the South Caucasus. The hosts hold a pro-Soviet view while guest scholars are more nuanced; it foregrounds both gains and losses."
  - title: "Working Class History"
    authors: ["Working Class History"]
    year: 2018
    format: podcast
    kind: series
    access: free
    url: "https://workingclasshistory.com/podcasts/"
    note: "Audio documentaries built from first-hand testimony of ordinary people in labor and social struggles worldwide, including inside state-socialist societies. Centers lived experience over state narratives."
  - title: "A State of Mind"
    authors: ["Daniel Gordon"]
    year: 2004
    format: film
    kind: documentary
    access: free
    length: "93 min"
    url: "https://watch.plex.tv/movie/a-state-of-mind"
    note: "Follows two young North Korean gymnasts preparing for the Mass Games. Notably observational and non-judgmental, a rare look at ordinary daily life inside the DPRK. Free with ads on Plex."
  - title: "North Koreans tell BBC they are stuck and waiting to die"
    authors: ["BBC News"]
    year: 2023
    format: video
    kind: report
    access: free
    length: "22 min"
    url: "https://www.youtube.com/watch?v=SiviOdWDl9o"
    note: "About 6.7 million views. Rare recent testimony smuggled out during the pandemic lockdown, on hunger and fear. A mainstream-critical counterweight to the observational film above."
  - title: "Sea of Memory: A Vietnamese Boat Refugee Documentary"
    authors: ["SuperNamn"]
    year: 2011
    format: video
    kind: documentary
    access: free
    length: "25 min"
    url: "https://www.youtube.com/watch?v=YNe897ereaY"
    note: "About 114,000 views. First-person memory of the boat people who fled reunified Vietnam. The testimony of those the revolution drove out, weighed alongside its defenders."
  - title: "The Death of Yugoslavia"
    authors: ["BBC"]
    year: 1995
    format: video
    kind: documentary
    access: free
    length: "287 min"
    url: "https://www.youtube.com/watch?v=bVUg-VoPAeA"
    note: "About 677,000 views. The protagonists themselves narrate the collapse of Yugoslav socialism into war. First-person disillusion and catastrophe, critical of the nationalist leaderships without a cold-war slant."
  - title: "Shanghai Journal: An Eyewitness Account of the Cultural Revolution"
    authors: ["Neale Hunter"]
    year: 1969
    format: primary-source
    kind: memoir
    access: borrow
    url: "https://archive.org/details/shanghaijournale0000hunt"
    note: "An Australian teacher in Shanghai through the January Storm of 1967, watching workers take over the city. Written close to the events and broadly sympathetic to what he saw, which makes it a useful contrast with the memoirs written after the disillusion set in."
  - title: "Blood Red Sunset: A Memoir of the Chinese Cultural Revolution"
    authors: ["Ma Bo"]
    year: 1995
    format: primary-source
    kind: memoir
    access: borrow
    url: "https://archive.org/details/bloodredsunsetme00mabo"
    note: "A Red Guard sent to Inner Mongolia, who believed and then did not, on what the movement did to the people in it and to the people it was aimed at. The insider account of Chinese revolutionary violence, from someone who took part."
  - title: "A Vietcong Memoir"
    authors: ["Truong Nhu Tang"]
    year: 1985
    format: primary-source
    kind: memoir
    access: borrow
    url: "https://archive.org/details/vietcongmemoir00trng"
    note: "A founder of the NLF and Justice Minister of the Provisional Revolutionary Government, who won the war and then watched Hanoi dissolve the southern revolution he had built, and left by boat. He never recants the war against the Americans, which makes his account of the aftermath much harder to dismiss than an exile polemic."
  - title: "The Vietnamese Gulag"
    authors: ["Doan Van Toai", "David Chanoff"]
    year: 1986
    format: primary-source
    kind: memoir
    access: borrow
    url: "https://archive.org/details/vietnamesegulag0000doan"
    note: "A student organizer against the Saigon regime who was jailed by the victors he had campaigned for. First-person testimony on the reeducation camps, from someone who had been on the revolution's side."
  - title: "Lost Years: My 1,632 Days in Vietnamese Reeducation Camps"
    authors: ["Tran Tri Vu"]
    year: 1988
    format: primary-source
    kind: memoir
    access: borrow
    url: "https://archive.org/details/lostyearsmy1632d0000tran"
    note: "Four and a half years inside the camps, published by Berkeley's Institute of East Asian Studies rather than a trade press. The most detailed free account of what reeducation meant day to day."
---

_Lived experience as a source of knowledge. What has the room been through? What do participants already know from the inside?_

See [[02-pedagogy#Experiential|the pedagogy document]] for what this lens does across all themes.

Few themes are as likely to divide a room as this one. Someone's grandparents may have built a revolution; someone else's may have fled one. The same society that ran a literacy campaign also ran a labor camp. This lens refuses the two easy stories, the socialist paradise and the totalitarian nightmare, and starts instead from testimony that holds the contradictions together. The point is not to reach a verdict but to hear what living through these societies actually felt like, from people on every side of it.

## Anchor questions

Sample prompts to open experiential work in this theme. Adapt to your room, and expect strong feeling. See the [[04-facilitator-guide|facilitator guide]] for general guidance on experiential prompts.

- What were you taught about "communist countries," by whom, and in what setting?
- If you have lived in or visited one of these states, what surprised you?
- Does anyone in your family carry a story of these societies, of gratitude, of loss, or both?
- Have you relied on something one of these states is known for, free healthcare, education, housing? What would its absence mean?
- Have you feared or fled a state that called itself socialist? What did it do?
- What do you want to know that the usual stories never told you?

## Materials that may help

Experiential work begins from the room, and this theme's room often holds the history directly. These materials extend that testimony, deliberately from opposing sides. All are free unless marked otherwise.

### Testimony from the Soviet world

- **Svetlana Alexievich** (b. 1948), _Secondhand Time_ (2013). Assembled testimony of ordinary people living through the Soviet collapse, holding pride, loss, terror, and longing together. The single best balanced source for the lens. No borrowable English copy exists. Paywalled: [penguinrandomhouse.com](https://www.penguinrandomhouse.com/books/541184/secondhand-time-by-svetlana-alexievich/). Her Nobel lecture, delivered as the book was being finished, is free and assignable: [nobelprize.org](https://www.nobelprize.org/prizes/literature/2015/alexievich/lecture/)
- **Nadezhda Mandelstam** (1899 to 1980), _Hope Against Hope_ (1970). The poet's widow on his arrest and death in the Great Purge. The necessary weight on the critical side. Free to borrow: [archive.org](https://archive.org/details/hopeagainsthopem00mand)
- **Reimagining Soviet Georgia** (2020 to present). Oral histories of daily life and post-socialism in the South Caucasus, holding gains and losses together. Free: [podcasts.apple.com](https://podcasts.apple.com/us/podcast/reimagining-soviet-georgia/id1555761205)
- **Working Class History** (2018 to present). First-hand testimony from ordinary people, including inside state-socialist societies. Free: [workingclasshistory.com](https://workingclasshistory.com/podcasts/)

### China, from inside

- **Neale Hunter**, _Shanghai Journal_ (1969). An Australian teacher in Shanghai through the January Storm of 1967, watching workers take the city. Written close to the events and broadly sympathetic, which is exactly why it is worth reading beside the memoirs written after the disillusion arrived: it records what it felt like while people still believed it. Free to borrow: [archive.org](https://archive.org/details/shanghaijournale0000hunt)
- **Ma Bo** (b. 1947), _Blood Red Sunset_ (1995). A Red Guard sent to Inner Mongolia who believed, and then did not. On what the movement did to the people inside it as well as to the people it was aimed at, by someone who did the aiming. Free to borrow: [archive.org](https://archive.org/details/bloodredsunsetme00mabo)

### Vietnam after the victory

The theme covers the war and stops. These are the people who won it and then had to live in what came next.

- **Truong Nhu Tang**, _A Vietcong Memoir_ (1985). A founder of the National Liberation Front and Justice Minister of the Provisional Revolutionary Government, who watched Hanoi dissolve the southern revolution he had spent his life building, and left by boat. He never recants the war against the Americans, and that is what makes his account of the aftermath so hard to wave away as an exile's grievance. Free to borrow: [archive.org](https://archive.org/details/vietcongmemoir00trng)
- **Doan Van Toai**, _The Vietnamese Gulag_ (1986). A student organizer against the Saigon regime, jailed after 1975 by the side he had campaigned for. Free to borrow: [archive.org](https://archive.org/details/vietnamesegulag0000doan)
- **Tran Tri Vu**, _Lost Years_ (1988). Four and a half years in the reeducation camps, day by day, published by Berkeley's Institute of East Asian Studies rather than a trade press. Free to borrow: [archive.org](https://archive.org/details/lostyearsmy1632d0000tran)

### Inside the Asian and Eastern socialist states

- **A State of Mind** (Daniel Gordon, 2004, 93 min). Two young North Korean gymnasts and their families, observational and non-judgmental. A rare look at ordinary DPRK life. Free with ads: [watch.plex.tv](https://watch.plex.tv/movie/a-state-of-mind)
- **"North Koreans tell BBC they are stuck and waiting to die"** (BBC, 2023). Roughly 22 minutes, about 6.7 million views. Rare recent testimony on hunger and fear, the mainstream-critical counterweight to the film above. [youtube.com/watch?v=SiviOdWDl9o](https://www.youtube.com/watch?v=SiviOdWDl9o)
- **Sea of Memory** (2011, 25 min). The testimony of the Vietnamese boat people who fled after 1975, the voices of those the revolution drove out. [youtube.com/watch?v=YNe897ereaY](https://www.youtube.com/watch?v=YNe897ereaY)
- **The Death of Yugoslavia** (BBC, 1995, 287 min). The protagonists narrate the collapse of Yugoslav socialism into war, first-person and unsparing. [youtube.com/watch?v=bVUg-VoPAeA](https://www.youtube.com/watch?v=bVUg-VoPAeA)

## A note on method

This is the theme where a facilitator most needs to protect the room. People arrive with inherited loyalties and inherited wounds, and a session can slide into a shouting match between "you're defending dictators" and "you're repeating cold-war propaganda." Name that risk out loud at the start. Insist that a literacy campaign and a labor camp can both be true, that an exile's grief and a worker's gratitude are both knowledge, and that the goal is to understand what happened, not to win. Testimony from every side has standing here.

## Status

This lens has a developed first pass for this theme. Resources continue to be added as the project grows. Contributions welcome, especially testimony from Chinese workers and peasants rather than students, teachers, and Red Guards: the China voices here are educated and urban, which is a real slant. Testimony from Laos and from Central Asia is absent entirely.
