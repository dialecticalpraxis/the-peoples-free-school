---
title: Historical
resources:
  - title: "Page Act"
    authors: ["U.S. Congress"]
    year: 1875
    format: primary-source
    kind: document
    access: free
    url: "https://loveman.sdsu.edu/docs/1875Immigration%20Act.pdf"
    note: "The first federal immigration restriction, barring 'Oriental' women on a presumption of prostitution. The racial border was drawn through women's bodies seven years before Chinese Exclusion. Two pages."
  - title: "Chinese Exclusion Act"
    authors: ["U.S. Congress"]
    year: 1882
    format: primary-source
    kind: document
    access: free
    url: "https://www.archives.gov/milestone-documents/chinese-exclusion-act"
    note: "The first US law to bar a group by name, and the template for every racialized immigration restriction that followed. Full transcript and an image of the original enrolled act."
  - title: "How the Irish Became White"
    authors: ["Noel Ignatiev"]
    year: 1995
    format: reading
    kind: book
    access: paywalled
    url: "https://www.routledge.com/How-the-Irish-Became-White/Ignatiev/p/book/9780415963091"
    note: "How Irish immigrants, despised as a race apart on arrival, claimed whiteness through the Democratic machine, the unions, and anti-Black violence. Whiteness as a membership sold on political terms. Also free to borrow on archive.org."
  - title: "Working Toward Whiteness: How America's Immigrants Became White"
    authors: ["David R. Roediger"]
    year: 2005
    format: reading
    kind: book
    access: paywalled
    url: "https://www.hachettebookgroup.com/titles/david-r-roediger/working-toward-whiteness/9781541673472/?lens=basic-books"
    note: "Follows southern and eastern European immigrants from 'inbetween people' to suburban whiteness, earned through housing, unions, and state policy rather than simply granted."
  - title: "How Jews Became White Folks and What That Says About Race in America"
    authors: ["Karen Brodkin"]
    year: 1998
    format: reading
    kind: book
    access: paywalled
    url: "https://www.rutgersuniversitypress.org/how-jews-became-white-folks-and-what-that-says-about-race-in-america/9780813525907"
    note: "Jews reclassified into whiteness after 1945 by the GI Bill, FHA loans, and suburbanization. An anthropologist's account braided with her own family's story, showing the state doing the reclassifying."
  - title: "How Jews Became White Folks, and May Become Nonwhite Under Trump"
    authors: ["Karen Brodkin"]
    year: 2016
    format: reading
    kind: article
    access: free
    url: "https://forward.com/opinion/356166/how-jews-became-white-folks-and-may-become-nonwhite-under-trump/"
    note: "Brodkin's own essay restating the book's argument: Jewish racial classification in America moved once and can move back. Free, and short enough to assign. This host bot-blocks automated checks but the page is live."
  - title: "Whiteness of a Different Color: European Immigrants and the Alchemy of Race"
    authors: ["Matthew Frye Jacobson"]
    year: 1998
    format: reading
    kind: book
    access: paywalled
    tier: extended
    url: "https://www.hup.harvard.edu/books/9780674951914"
    note: "Whiteness itself repeatedly redrawn: from a broad 1790 'free white persons' to a fractured hierarchy of Celts and Hebrews and Mediterraneans and back to a consolidated Caucasian."
  - title: "Whiteness and the Normative American Citizen"
    authors: ["Matthew Frye Jacobson"]
    year: 2014
    format: video
    kind: lecture
    access: free
    length: "123 min"
    url: "https://www.youtube.com/watch?v=r_WbWd4fw4g"
    note: "About 4,400 views, a very niche pick kept because it is the author unabridged: an Organization of American Historians lecture on how whiteness was built and renegotiated for European immigrants, which is the book."
  - title: "Immigration Act of 1924 (Johnson-Reed Act)"
    authors: ["U.S. Congress"]
    year: 1924
    format: primary-source
    kind: document
    access: free
    url: "https://immigrationhistory.org/item/1924-immigration-act-johnson-reed-act/"
    note: "The national-origins quota that fixed the racial composition of the nation by law, pegged to the 1890 census precisely to exclude southern and eastern Europeans, and barring Asians outright. The hinge of this theme."
  - title: "Executive Order 9066"
    authors: ["Franklin D. Roosevelt"]
    year: 1942
    format: primary-source
    kind: document
    access: free
    url: "https://www.archives.gov/milestone-documents/executive-order-9066"
    note: "The order authorizing mass removal. Worth reading for what it does not say: it never names Japanese Americans once. Scan and transcript."
  - title: "Korematsu v. United States, 323 U.S. 214"
    authors: ["U.S. Supreme Court"]
    year: 1944
    format: primary-source
    kind: document
    access: free
    url: "https://www.law.cornell.edu/supremecourt/text/323/214"
    note: "The Court upholding racial incarceration. Assign Murphy's dissent, which calls it what it is: 'the legalization of racism.'"
  - title: "Mexican Farm Labor Agreement (the Bracero Agreement)"
    authors: ["United States", "Mexico"]
    year: 1942
    format: primary-source
    kind: document
    access: free
    url: "https://immigrationhistory.org/item/bracero-agreement/"
    note: "The guest-worker treaty promising a thirty-cent minimum wage, decent camps, and no discrimination, and delivering none of it. Read the promises against Galarza and the Bracero archive."
  - title: "Operation Wetback"
    authors: ["Immigration History"]
    year: 1954
    format: primary-source
    kind: document
    access: free
    url: "https://immigrationhistory.org/item/operation-wetback/"
    note: "The mass-deportation campaign run in the same years and the same fields as the Bracero program. Importation and expulsion were one labor policy, not two."
  - title: "The Color of Success: Asian Americans and the Origins of the Model Minority"
    authors: ["Ellen D. Wu"]
    year: 2013
    format: reading
    kind: book
    access: paywalled
    url: "https://press.princeton.edu/books/paperback/9780691168029/the-color-of-success"
    note: "The turn from 'yellow peril' to 'model minority' across incarceration, Hawaii statehood, and the Cold War, showing the model minority was manufactured as a weapon against Black claims rather than earned."
  - title: "Impossible Subjects: Illegal Aliens and the Making of Modern America"
    authors: ["Mae M. Ngai"]
    year: 2004
    format: reading
    kind: book
    access: paywalled
    url: "https://press.princeton.edu/books/paperback/9780691160825/impossible-subjects"
    note: "How the 1924 quota regime manufactured the 'illegal alien' as a permanent caste, racializing Mexicans and Asians through immigration law itself. The law as a race-making machine."
  - title: "Undocumented Migration to the U.S.: Origins, Consequences, Remedies"
    authors: ["Mae Ngai"]
    format: video
    kind: lecture
    access: free
    length: "76 min"
    url: "https://www.youtube.com/watch?v=JQl01iLxpLQ"
    note: "About 1,000 views, which is very niche, and it is here only because it is Ngai giving the book's thesis in full: the 1920s quotas closed the door and manufactured the category of the illegal alien."
  - title: "Immigration and Nationality Act of 1965 (Hart-Celler Act)"
    authors: ["U.S. Congress"]
    year: 1965
    format: primary-source
    kind: document
    access: free
    url: "https://immigrationhistory.org/item/hart-celler-act/"
    note: "Abolished the national-origins quota and remade American demography, while introducing hemispheric caps that manufactured Mexican illegality at scale. The liberal reform that created the modern undocumented population."
  - title: "The Chinese Exclusion Act"
    authors: ["Ric Burns", "Li-Shin Yu"]
    year: 2018
    format: film
    kind: documentary
    access: free
    length: "160 min"
    url: "https://www.pbs.org/wgbh/americanexperience/films/chinese-exclusion-act/"
    note: "A feature documentary on the only US law to bar a named group, and what it did to the communities already here. Streams free on PBS and pairs directly with the Act's text."
  - title: "A Latinx Resource Guide: the Bracero Program"
    authors: ["Library of Congress"]
    format: reading
    kind: collection
    access: free
    tier: extended
    url: "https://guides.loc.gov/latinx-civil-rights/bracero-program"
    note: "A curated primary-source guide to the guest-worker program, for a group that wants to research the period itself rather than read about it."
---

_How did this come to be? What were the conditions, struggles, and contingencies that produced the current arrangement?_

See [[02-pedagogy#Historical|the pedagogy document]] for what this lens does across all themes.

This is a history of laws. Not because law is where life happens, but because in this story the statute is the machine: who may enter, who may naturalize, who may be sent back, and who is a permanent guest. Read in order, the acts below show race being manufactured in public, on the record, by recorded votes. The Page Act draws the line through women's bodies; Chinese Exclusion names a people; the 1924 quota pegs the nation to the 1890 census so that the wrong Europeans cannot come; 9066 empties the coast without naming anyone; Bracero imports the workers that Operation Wetback expels in the same fields the same year; and 1965, the liberal reform, invents the modern undocumented population by accident. It runs alongside [[03-themes/anti-blackness-and-racial-capitalism/index|Anti-Blackness and Racial Capitalism]], which traces the other mechanism.

## Anchor questions

- The Irish, Italians, and Jews arrived as despised races and are now simply white. Who let them in, and what did admission cost, and who paid it?
- Why does the law keep drawing the line in different places? What does that tell you about the line?
- Bracero imported workers while Operation Wetback deported them, in the same fields, in the same years. How is that one policy?
- 1965 was the liberal victory that abolished the racist quota. How did it produce the modern "illegal immigrant"?
- Executive Order 9066 never says "Japanese." Why not, and what does the omission accomplish?

## Materials

Free unless marked otherwise. "Free to borrow" means a free Internet Archive account and a renewable loan.

### The expansion of whiteness

Whiteness was not a fact about complexions. It was a membership, and the door opened.

- **Noel Ignatiev** (1940 to 2019), _How the Irish Became White_ (1995). The Irish arrived despised as a race apart and became white by buying into the Democratic machine, the trade unions, and anti-Black violence. Whiteness here is not a complexion but a membership, sold on political terms and paid for at someone else's expense. No borrowable copy exists. Paywalled: [routledge.com](https://www.routledge.com/How-the-Irish-Became-White/Ignatiev/p/book/9780415963091), and see [[04-resources/index#A note on access|the note on access]] for how to get it. Ignatiev on the same politics, free and hosted by his own publisher: [blog.pmpress.org](https://blog.pmpress.org/2019/10/24/noel-ignatiev-on-race-traitor-white-supremacy-and-abolition-on-its-going-down/)
- **David R. Roediger** (b. 1952), _Working Toward Whiteness_ (2005). Southern and eastern Europeans move from "inbetween people" to suburban whiteness through housing policy, unions, and the state. The companion to his _Wages of Whiteness_, which sits in the anti-Blackness theme. No borrowable copy exists. Paywalled: [hachettebookgroup.com](https://www.hachettebookgroup.com/titles/david-r-roediger/working-toward-whiteness/9781541673472/?lens=basic-books). Roediger lays out the thesis in a free interview published by the UE union: [ueunion.org](https://www.ueunion.org/ue-news-feature/2012/race-and-the-working-class-an-interview-with-david-roediger)
- **Karen Brodkin**, _How Jews Became White Folks_ (1998). How the GI Bill, FHA loans, and the suburbs reclassified Jews as white after 1945, told by an anthropologist through her own family. The clearest case of the state doing the reclassifying. No borrowable copy exists. Paywalled: [rutgersuniversitypress.org](https://www.rutgersuniversitypress.org/how-jews-became-white-folks-and-what-that-says-about-race-in-america/9780813525907). Brodkin restated the argument herself in a free essay: [forward.com](https://forward.com/opinion/356166/how-jews-became-white-folks-and-may-become-nonwhite-under-trump/)

### The law as a race-making machine

- **Ellen D. Wu**, _The Color of Success_ (2013). The turn from "yellow peril" to "model minority" runs through wartime incarceration, Hawaii statehood, and Cold War image-management. The model minority was manufactured, and manufactured as a weapon against Black claims. No borrowable copy exists. Paywalled: [press.princeton.edu](https://press.princeton.edu/books/paperback/9780691168029/the-color-of-success)
- **Mae M. Ngai**, _Impossible Subjects_ (2004). The 1924 quota regime did not just restrict immigration, it produced the "illegal alien" as a permanent caste, racializing Mexicans and Asians through the law itself. No borrowable copy exists. Paywalled: [press.princeton.edu](https://press.princeton.edu/books/paperback/9780691160825/impossible-subjects). Ngai gives the thesis in a free lecture, extremely niche at about 1,000 views but the author unabridged: [youtube.com](https://www.youtube.com/watch?v=JQl01iLxpLQ)

### Primary sources: read the statutes

The whole argument of this theme is available in the documents. They are short, and they say what they are doing.

- **Page Act** (1875). The first federal restriction, barring "Oriental" women on a presumption of prostitution. Two pages, and the racial border is drawn through women's bodies seven years before anyone is barred by name. Free: [loveman.sdsu.edu](https://loveman.sdsu.edu/docs/1875Immigration%20Act.pdf)
- **Chinese Exclusion Act** (1882). The first US law to bar a group by name. Free: [archives.gov](https://www.archives.gov/milestone-documents/chinese-exclusion-act)
- **Immigration Act of 1924** (Johnson-Reed). The national-origins quota, pegged to the 1890 census for the express purpose of keeping the composition of the country as it had been before the wrong Europeans arrived. Free: [immigrationhistory.org](https://immigrationhistory.org/item/1924-immigration-act-johnson-reed-act/)
- **Executive Order 9066** (1942). Read it and notice it never says "Japanese." Free: [archives.gov](https://www.archives.gov/milestone-documents/executive-order-9066)
- **Korematsu v. United States** (1944). The Court blessing it. Assign Murphy's dissent, which names it "the legalization of racism," and ask why the majority could not see what he saw. Free: [law.cornell.edu](https://www.law.cornell.edu/supremecourt/text/323/214)
- **Mexican Farm Labor Agreement** (1942) and **Operation Wetback** (1954). Read together, or they make no sense: the state imported the workers it was simultaneously deporting, in the same fields, in the same decade. Free: [the Bracero agreement](https://immigrationhistory.org/item/bracero-agreement/) and [Operation Wetback](https://immigrationhistory.org/item/operation-wetback/)
- **Immigration and Nationality Act of 1965** (Hart-Celler). The reform that ended the racist quota, and, through hemispheric caps, manufactured Mexican illegality at scale. The best argument in the theme against reading history as progress. Free: [immigrationhistory.org](https://immigrationhistory.org/item/hart-celler-act/)

### Film

- **The Chinese Exclusion Act** (Ric Burns and Li-Shin Yu, PBS American Experience, 2018, 160 min). The only US law to bar a named people, and what it did to those already here. Streams free: [pbs.org](https://www.pbs.org/wgbh/americanexperience/films/chinese-exclusion-act/)

<!-- further-reading:start -->

## Further reading

_For resources behind a paywall, see [[04-resources/index#A note on access|the note on access]]._

- **Matthew Frye Jacobson**, _Whiteness of a Different Color: European Immigrants and the Alchemy of Race_ (1998). Whiteness itself repeatedly redrawn: from a broad 1790 'free white persons' to a fractured hierarchy of Celts and Hebrews and Mediterraneans and back to a consolidated Caucasian. Not free: [hup.harvard.edu](https://www.hup.harvard.edu/books/9780674951914)
- **Library of Congress**, _A Latinx Resource Guide: the Bracero Program_. A curated primary-source guide to the guest-worker program, for a group that wants to research the period itself rather than read about it. Free: [guides.loc.gov](https://guides.loc.gov/latinx-civil-rights/bracero-program)

<!-- further-reading:end -->

## Status

This lens has a developed first pass for this theme. Resources continue to be added as the project grows. Contributions welcome, especially on Filipino racialization and the colonial ambiguity of US nationals, the 1930s expulsion of Mexicans and Mexican Americans (many of them citizens), and immigration and race outside the United States.
